...
- [ ] **GREEN/integration:** `Run-VgTests 't02-green' 'FullyQualifiedName~AuditMediaRegressionTests|FullyQualifiedName~Preview12FinalizationTests|FullyQualifiedName~DownloadFailureRegressionTests|FullyQualifiedName~AuthenticatedHlsDownloadIntegrationTests'`. Adjust old recovery expectations explicitly to failure plus retained partial. Extend the authenticated HLS fixture with unavailable final segment; real yt-dlp must fail and preserve the owned incomplete file. Complete control must have requested streams, expected duration, ffprobe height and `ffmpeg -v error -nostdin -i output -f null -` exit 0, including tail decode.
...
**Files:** Modify `src/VideoGrabber.App/MainWindow.DevTools.cs`, `MainWindow.Browser.cs`, `MainWindow.BatchDownload.cs`; create `src/VideoGrabber.Infrastructure/Browser/MediaCandidateMerge.cs`; extend Infrastructure tests `MediaCandidatePresentationTests.cs`, `BrowserWiringRegressionTests.cs`; create `BrowserSelectionRegressionTests.cs` there.
...
- [ ] **RED:** Add a real reducer test to `BrowserSelectionRegressionTests` and a GUI reproduction record (select PART 4, reverse arrivals, late metadata). For merge use existing `MediaCandidate`/`HlsManifestInfo` constructors:
...
// Rebuild under the selection/quality suppression flag.
_mediaCandidatesBox.SelectedItem = _mediaCandidatesBox.Items.OfType<ComboBoxItem>()
    .FirstOrDefault(item => item.Tag is MediaCandidate value && value.Source == selectedSource);
```
...
- [ ] **Review/commit:** Stage seven listed files; commit `fix: retain browser selection and confirmed metadata`.
...
**Files:** Create `src/VideoGrabber.Core/Downloads/DownloadQuality.cs`; modify `src/VideoGrabber.Infrastructure/Browser/HlsTrackSelector.cs`, `BrowserDownloadQueue.cs`, `MediaDownloadPlanResolver.cs`, `src/VideoGrabber.Infrastructure/Downloads/YtDlpDownloader.cs`, `src/VideoGrabber.App/MainWindow.BatchDownload.cs`, `MainWindow.Download.cs`; extend Infrastructure tests `AuditBrowserRegressionTests.cs`, `HlsTrackSelectorTests.cs`, `YtDlpDownloaderTests.cs`, `AuditSixPartsIntegrationTests.cs`.

**Interfaces:** `public readonly record struct DownloadQuality(int? MaximumHeight)` with `TryParse(string? value, out DownloadQuality quality): bool`, `ToTag(): string`; best=null, `4K`→2160, positive ASCII integer + `p` accepted through Int32; invalid tags reject instead of silently selecting best. Keep `DownloadRequest.Quality` string for compatibility and normalize once. Task 2's `ResolvedHlsLeaf` means the verified leaf was already selected; `SelectFormat` uses `best` (or `bestaudio/best` for requested audio) without a height expression. Append `DownloadRequest? capturedRequest = null` to the private `DownloadSourceAsync` boundary, constructed once before async preparation, so browser quality/audio/output never reread global controls.
...
- [ ] **GREEN implementation:** Replace both hardcoded allowlists with the shared parser; set IsResolved/ResolvedHlsLeaf from actual plan. Capture request in `DownloadCandidateAsync` before first await and pass it intact; Task 10 adds lifetime ownership around that boundary. Numeric non-leaf selectors interpolate only parsed integer, never raw text. Persist queue tag unchanged for valid dynamic quality.
...
- [ ] **Review/commit:** Stage eleven named files; commit `fix: preserve one resolved media quality choice`.
...
- [ ] **GREEN implementation:** Begin operation before `RefreshCandidateBindingAsync`/preflight's first await; keep Task 7's immutable captured request. Pass the existing operation through download so it does not reject its own preflight owner. Link page and window cancellation. Convert preflight timeout/network failures to one visible Failed result. Common `CompleteOperation` clears CTS, restores controls and dispatches exactly one action returned by coordinator. All download/trim/join/MP3/Whisper finally blocks call it. Queue continuation after successful media operation is allowed only if the user explicitly requested it; any close wins.

```csharp
switch (coordinator.Complete(outcome))
{
...
- [ ] **GREEN:** Run `OperationCoordinatorTests|BatchDownloadWiringTests|BrowserWiringRegressionTests|CallbackFailureRegressionTests`. Cover success, Cancel, 401,403, timeout, double click, close during each media action, close+pending queue and stale progress. Pending entries stay present; no new call after Cancel/failure. GUI gate proves the window actually closes once after process termination and controls return to usable states.
- [ ] **Review/commit:** Stage eight files; commit `fix: unify preflight and media operation lifetime`.

### Task 11: Preserve queued work across navigation without stale session reuse

**Files:** Modify `src/VideoGrabber.Infrastructure/Browser/BrowserDownloadQueue.cs`, `src/VideoGrabber.App/MainWindow.BatchDownload.cs`, `MainWindow.DevTools.cs`, `MainWindow.Browser.cs`; extend Infrastructure tests `BrowserDownloadQueueTests.cs`, `BrowserSessionTests.cs`.
...
- [ ] **RED:** Add a queue test with captured A context, navigate/reset discovery to B through a small fake UI host using Task 10 coordinator, and assert entries unchanged. Core queue snapshot assertions:
...
- [ ] **GREEN:** `Run-VgTests 't11-green' 'FullyQualifiedName~BrowserDownloadQueueTests|FullyQualifiedName~BrowserSessionTests|FullyQualifiedName~OperationCoordinatorTests'`. A→B preserves source/order/quality/title; same-session Continue uses A source snapshot; logout/session switch causes zero downloader calls for old entries; no persistence claim on restart.
- [ ] **Review/commit:** Stage six files; commit `fix: separate browser queue and navigation lifetime`.

### Task 12: Reliable process-tree fixture, then owned-tree cancellation

**Files:** First modify only `tests/VideoGrabber.Infrastructure.Tests/AuditNetworkRegressionTests.cs`; create `tests/VideoGrabber.Infrastructure.Tests/Fixtures/ProcessTreeFixture.ps1`. After behavioral RED modify `src/VideoGrabber.Infrastructure/Processes/ProcessRunner.cs`; create `WindowsProcessJob.cs` there; extend Infrastructure tests `ProcessLifecycleTests.cs`, `CallbackFailureRegressionTests.cs`.

**Interfaces:** Keep `IProcessRunner.RunAsync(ProcessSpec, Action<string>?, CancellationToken)` and ProcessResult. `WindowsProcessJob` owns a Windows Job Object with KILL_ON_JOB_CLOSE, implements IDisposable; `Start(ProcessStartInfo info): Process` launches the root assigned to this job before it can create a descendant (suspended creation/assignment/resume or equivalent verified atomic launch); no `Process.Start` then racy assignment. Independent normal-exit drain deadline=2 seconds, cleanup deadline=5 seconds. Preserve user cancellation as OperationCanceledException, overall process timeout as TimeoutException, callback failure as original exception.
...
- [ ] **GREEN:** Run `AuditNetworkRegressionTests` process filter plus `ProcessLifecycleTests|ProcessRunnerSafetyTests|CallbackFailureRegressionTests`. Require parent-exit/child-alive fixture, no surviving descendant after cancel/normal-drain-timeout/callback error, bounded completion and correct classification. Repeated 20 iterations exercise assignment race. Success output tail and standard UTF-8 parsing remain intact.
- [ ] **Review/commit:** Stage six files; commit `fix: own subprocess trees through process completion`.

### Task 13: Component installation joins operation lifetime

**Files:** Modify `src/VideoGrabber.App/MainWindow.xaml.cs`, `MainWindow.Download.cs`; create `src/VideoGrabber.Infrastructure/Components/ComponentInstaller.cs`; create `tests/VideoGrabber.Infrastructure.Tests/ComponentInstallerTests.cs`.

**Interfaces:** `ComponentInstaller(IProcessRunner runner).InstallAsync(string script, string destination, CancellationToken token): Task<ProcessResult>` creates a noninteractive powershell ProcessSpec with 10-minute timeout and delegates to Task 12's runner. `MainWindow.InstallComponentsAsync(bool forceUpdate, CancellationToken cancellationToken)` receives existing download token or a manually owned operation token, never nests an unrelated owner. Manual operation uses Task 10's coordinator; `_isInstallingComponents` remains status, not sole ownership.
...
- [ ] **GREEN implementation:** Route install through ComponentInstaller and common operation completion. Closing cancels and waits for owned process tree; installer task is observed. Catch cancellation separately, retain timeout/error messages as sanitized failures, refresh component status only after successful result. Task 17 provides transactional on-disk behavior; until it lands do not run actual updates on installed tools.
...
- [ ] **Review/commit:** Stage four files; commit `fix: cancel and await component installation`.

### Task 14: Mandatory egress guard independently of routing

**Files:** Modify `src/VideoGrabber.Core/Security/UrlPolicy.cs`, `src/VideoGrabber.Infrastructure/Networking/RouteConnector.cs`, `SiteRouteProxy.cs`, `src/VideoGrabber.Infrastructure/Downloads/YtDlpDownloader.cs`, `src/VideoGrabber.Infrastructure/Browser/HlsPreflightClient.cs`, `src/VideoGrabber.App/MainWindow.Network.cs`; create `src/VideoGrabber.Infrastructure/Networking/DownloadEgressSession.cs`; create Infrastructure test `DownloadEgressTests.cs`; extend Core `UrlPolicyTests.cs` and Infrastructure `RoutingDownloadTests.cs`, `AuthenticatedHlsDownloadIntegrationTests.cs`.

**Interfaces:** `DownloadEgressSession` implements IDisposable, exposes `ProxyUrl: string`, uses RouteConnector validated DNS→pinned IP connect for every SOCKS CONNECT. Default constructor path builds policy with no adapter rule; adapter choice is independent. Append optional `Func<DownloadEgressSession>? egressFactory = null` to YtDlpDownloader constructor; default creates a mandatory session when request has no managed proxy. A supplied proxy must come from the app's managed session; raw external caller proxies are not accepted as proof of enforcement. Test-only factory permits named owned listener endpoints, never global URL-policy bypass. Consolidate literal public-IP policy into Core for UrlPolicy and RouteConnector; mapped IPv6/ULA/loopback/private/reserved are rejected after normalization.
...
- [ ] **GREEN implementation:** EnsureRoutingProxy always supplies managed egress even with no adapter rule; keep selected interface binding and session lease checks. Downloader entry points (ordinary, direct manifest, split tracks) cannot skip guard. Manual HLS preflight uses mandatory pinned connector too. Reject unsupported protocols before invoking tools and prevent native-tool fallback to unsupported downloaders; FFmpeg only merges/edits locally owned paths with network protocols absent.
- [ ] **GREEN:** `Run-VgTests 't14-green' 'FullyQualifiedName~DownloadEgressTests|FullyQualifiedName~RoutingDownloadTests|FullyQualifiedName~SiteRouteTests|FullyQualifiedName~SiteProxyTests|FullyQualifiedName~AuthenticatedHlsDownloadIntegrationTests'`; separately `& $vgDotnet test tests/VideoGrabber.Core.Tests/VideoGrabber.Core.Tests.csproj -c Release --no-restore --filter FullyQualifiedName~UrlPolicyTests`. Positive public target succeeds through pinned connector, DNS answer change cannot move connection, default/routed nested private requests produce zero hits; direct and split HLS and local FFmpeg work. Record extractor compatibility limitations explicitly.
- [ ] **Review/commit:** Stage eleven files; commit `fix: enforce downloader egress on every supported path`.
...
**Interfaces:** `SrtValidator.TryValidate(string text, double mediaDurationSeconds, out string? error): bool`. Limits: 16 MiB UTF-8 file before read, 100,000 cues, 65,536 characters per cue. Accept CRLF/LF, BOM, Unicode speech and multi-line cue text. Require >=1 nonempty cue, valid `HH:MM:SS,mmm` with minutes/seconds<60, strictly increasing cue starts, end>start, non-overlapping intervals, cue end <= media duration+1 second when duration known. Keep unvalidated transcript/SRT in owned ASR directory on failure, do not promote either file; diagnostics contain only error kind/count, never transcript text.
...
**Files:** Modify `scripts/Install-Components.ps1`; create `scripts/Test-InstallComponents.ps1`; modify `src/VideoGrabber.Infrastructure/Components/ToolLocator.cs`; extend Infrastructure tests `ToolLocatorTests.cs`, `ComponentInstallerTests.cs`.
...
- [ ] **GREEN implementation:** Download/stage/verify all before pointer commit; no current-path Copy-Item -Force per tool. Write a transaction journal with old/new relative paths and stage status, then atomically select generation. If pointer replace or post-selection self-test fails, restore previous pointer; keep a readable recovery journal if rollback itself fails, refuse readiness instead of mixing tools. Missing trusted digest blocks selection; local computed hash alone is not provenance. Cancellation before commit preserves old set; cancellation during bounded commit finishes rollback before returning. Process termination/crash restart replays journal to last verified set.
- [ ] **GREEN:** `powershell.exe -NoProfile -NonInteractive -File scripts/Test-InstallComponents.ps1`; `Run-VgTests 't17-green' 'FullyQualifiedName~ToolLocatorTests|FullyQualifiedName~ComponentInstallerTests'`. Include second-download failure, wrong digest, missing ffprobe, copy denied, pointer lock, self-test failure, cancellation, crash before/after pointer replace and successful upgrade/reopen. Original hashes unchanged for all precommit failures; no orphan process; old set recoverable. Do not execute installer against `%LOCALAPPDATA%\VideoGrabber\tools`.
- [ ] **Review/commit:** Stage five files; commit `fix: activate component updates as verified sets`.
