...
public static class MediaDownloadPlanResolver
{
    ...
    {
        if (!DownloadQuality.TryParse(quality, out var parsedQuality))
            return Unresolved(candidate, "Некорректное качество видео.");
        ...
        if (plan is null || (parsedQuality.MaximumHeight is int height && plan.Video.Height != height)
        ...
        if (plan.Audio is not null)
            return new(candidate.Source, plan.Video.Uri, plan.Audio.Uri, true, ResolvedHlsLeaf: true);
