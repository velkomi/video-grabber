...
- YouTube/Shorts теперь запрашиваются через связку `mweb,default`, а не только `mweb`: это обходит текущий YouTube SABR-only rollout, при котором отдельные anonymous mweb-сессии возвращают только storyboard без видеоформатов;
- тот же fallback применяется и на этапе анализа, и в server worker, поэтому найденные 360p/480p/720p/1080p форматы остаются доступными при фактической загрузке.

### Проверено

- на production social-egress без YouTube cookies тестовый Shorts снова отдаёт реальные форматы вплоть до 2160p; загрузка 360p завершилась MP4 360x640 с аудио;
- Instagram Reel, актуальный TikTok и Pinterest video pin продолжают определяться без входа в эти сервисы.

## [0.1.10-preview.39-rc.1] - 2026-09-26

### Исправлено

- качество вертикальных Shorts/Reels/TikTok теперь выбирается по ориентационно-независимому разрешению yt-dlp, а не по буквальному height; 360p для вертикального ролика корректно означает 360x640;
- из списка качеств исключены storyboard и прочие не-видео форматы, из-за которых появлялись ложные варианты вроде 180p и server job уходил в review_required;
- server worker применяет штатную сортировку yt-dlp res:<N>, поэтому один и тот же выбор 360p/720p/1080p корректно работает и для горизонтального, и для вертикального видео.

### Проверено

- публичный YouTube Shorts проходит через social-egress + Deno + bgutil PO-token без cookies и без входа;
- публичный Instagram Reel и Pinterest video pin определяются через тот же anonymous social pipeline;
- актуальный публичный TikTok определяется через social-egress и curl_cffi impersonation без cookies; отдельные старые/заблокированные посты корректно остаются недоступными на стороне TikTok.
...
- серверный анализ ссылок больше не запускает Python-версию yt-dlp в образе без Python: API-образ содержит Python 3;
- API и server worker получили Node.js и явно передают yt-dlp JavaScript runtime node, что повышает совместимость с современным YouTube;
- ошибки YouTube классифицируются как недоступный ролик, обязательный вход, временный rate limit или ошибка анализа вместо технического yt-dlp source analysis failed;
- веб-интерфейс прямо сообщает, что неуспешный анализ ссылки не расходует бесплатную загрузку;
- Windows desktop worker после истечения access token сначала обновляет сессию и продолжает работу, а не становится offline до ручного входа.

### Обслуживание

- освобождён один legacy Free-резерв старого desktop-worker задания, завершившегося до доставки результата; ledger содержит парные reserve/release и баланс восстановлен до 10/0.
